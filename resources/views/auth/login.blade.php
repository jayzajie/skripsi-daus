...
<head>
    ...
    <title>Login - Sistem Informasi Inventori Surat dan Pelayanan SKTM</title>
...
<body class="min-h-screen bg-white font-sans text-slate-900 antialiased">
    <main class="flex min-h-screen items-center justify-center px-4 py-6">
        <section class="relative flex min-h-[720px] w-full max-w-[1440px] overflow-hidden rounded-xl border border-slate-200 bg-[#f4f8ff] shadow-[0_2px_16px_rgba(15,23,42,0.14)]">
            ...
            <div class="relative flex min-h-full flex-1 flex-col items-center justify-center px-5 py-8 lg:px-14">
                <div class="w-full max-w-[580px] rounded-xl bg-white px-10 py-12 shadow-[0_8px_22px_rgba(15,23,42,0.12)] sm:px-14 lg:px-[52px]">
                    <div class="flex flex-col items-center text-center">
                        <div class="flex h-[110px] w-[110px] items-center justify-center rounded-full bg-[#e7f0ff]">
                            <svg viewBox="0 0 80 80" class="h-[58px] w-[58px]" aria-hidden="true">
                                <path d="M24 36V25c0-9 7-16 16-16s16 7 16 16v11" fill="none" stroke="#1f78d1" stroke-width="8" stroke-linecap="round" />
                                <rect x="20" y="33" width="40" height="35" rx="4" fill="#1f78d1" />
                                <circle cx="40" cy="49" r="5" fill="#fff" />
                                <path d="M40 53v8" stroke="#fff" stroke-width="4" stroke-linecap="round" />
                            </svg>
                        </div>
                        <h2 class="mt-6 text-[30px] font-extrabold leading-tight text-[#16283d]">Login</h2>
                        <p class="mt-3 text-[17px] font-medium text-slate-600">Silakan masuk untuk mengakses sistem</p>
                    </div>

                    @if (session('status'))
                        <div class="mt-8 rounded-md border border-blue-100 bg-blue-50 px-4 py-3 text-sm font-medium text-blue-700">
                            {{ session('status') }}
                        </div>
                    @endif

                    <form method="POST" action="{{ route('login') }}" class="mt-9 space-y-7">
                        @csrf

                        <div>
                            <label for="email" class="block text-[16px] font-bold text-slate-800">Username</label>
                            <div class="mt-3 flex h-[58px] items-center rounded-md border border-slate-300 bg-white px-5 shadow-[inset_0_1px_2px_rgba(15,23,42,0.04)] transition focus-within:border-blue-500 focus-within:ring-2 focus-within:ring-blue-100">
                        ...
                        @if (Route::has('register'))
                            <p class="text-center text-[15px] font-medium text-slate-600">
                                Belum punya akun?
                                <a href="{{ route('register') }}" class="font-bold text-[#2d6bb4] hover:text-blue-800 focus:outline-none focus:ring-2 focus:ring-blue-500">
                                    Daftar
                                </a>
                            </p>
                        @endif
